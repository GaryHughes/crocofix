...
TEST_CASE("Order", "[order]") {
    ...
    SECTION("NewOrderSingleAndExecutionReports")
    {
        ...
        REQUIRE(order.OrdStatus() == crocofix::FIX_5_0SP2::field::OrdStatus::New);
        REQUIRE(order.CumQty().value() == "0");
        REQUIRE(order.AvgPx().value() == "0");

        order.update(decode_message(report_partial));

        REQUIRE(order.OrdStatus() == crocofix::FIX_5_0SP2::field::OrdStatus::PartiallyFilled);
        REQUIRE(order.CumQty().value() == "9107");
        REQUIRE(order.AvgPx().value() == "20");

        order.update(decode_message(report_filled));

        REQUIRE(order.OrdStatus() == crocofix::FIX_5_0SP2::field::OrdStatus::Filled);
