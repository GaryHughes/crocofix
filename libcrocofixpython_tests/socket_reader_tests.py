...
# This is a mock socket.socket, the count argument to recv() is ignored and
# it returns successive chunks of data from an array passed to the constructor. This
# lets the caller specify the recv() behaviour simply for test code.
class TestReader:
...
class TestSocketReader(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_complete_message_in_single_read(self):
        ...
        messages = []
        reader = SocketReader(TestReader(chunks))
        reader.message_read = lambda message : messages.append(message)
        reader.read_messages()
        self.assertEqual(len(messages), 1)

    def test_complete_message_in_two_pieces_aligned_on_a_field_boundary(self): 
        chunks = [ 
    ...
    def test_multiple_complete_messages_in_single_read(self):
